...
![The fixed IAM role with minimal policy attached](images/11-fixed-iam-role-minimal-policy.png)
*`bedrock-agent-tool-role-d1jun17` after the fix. `AmazonS3FullAccess` is gone, replaced
by `MinimalS3ReadOnlyCustomerRecords` — only `s3:GetObject` and `s3:ListBucket`, scoped
to exactly the `customer-records-d1jun17` bucket ARN. Nothing else.*
...
![Agent response after the fix - authorization error](images/12-accessdenied-response.png)
*Same exact injection message. This time the agent reports: "I encountered an
authorization error while trying to delete the objects... it seems the IAM role used
by the agent does not have the necessary permissions... please check the permissions
for the IAM role 'bedrock-agent-tool-role-d1jun17'." The model correctly diagnosed
the cause — but the point is it didn't matter whether it understood why. The
deletion simply could not happen.*

### 2. Confirm the bucket is untouched

**S3 Console →** `customer-backups-prod-YOUR_SUFFIX` should still show all its original files.

> **A note on this screenshot:** the bucket below shows 0 objects — but that's because
> the vulnerable-half attack already deleted everything in it earlier, and this lab run
> didn't re-upload the dummy files before testing the fix. This screenshot does *not*
> prove the fix protected files that were at risk in this particular run — it just
> confirms the second attack attempt added no further damage to an already-empty bucket.
> **To properly demonstrate this step yourself:** re-upload a few dummy files to
> `customer-backups-prod-YOUR_SUFFIX` before re-running the attack, then check the
> bucket afterward — you should see the files still present, which is the real proof
